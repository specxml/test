...
if [ $link1_status = "200" ] ; then 
	sudo wget -O /mediabots/WS2012R2.ISO http://download.microsoft.com/download/0/D/4/0D41501C-EEBC-4481-8A37-14A56A64BFDA/14393.0.161119-1705.RS1_REFRESH_SERVER_EVAL_X64FRE_RU-RU.ISO 
elif [ $link2_status = "200" -o $link2_status = "301" -o $link2_status = "302" ] ; then 
	sudo wget -P /mediabots http://download.microsoft.com/download/0/D/4/0D41501C-EEBC-4481-8A37-14A56A64BFDA/14393.0.161119-1705.RS1_REFRESH_SERVER_EVAL_X64FRE_RU-RU.ISO
else
...
sudo wget -P /floppy https://archive.org/download/prowinx-64/PROWinx64.exe  # Intel Network Adapter for Windows Server 2012 R2 
...
if [ $availableRAM -ge 4650 ] ; then # opened 2nd if
	...
	if [ ! -z $deleteLinux ] && [ $deleteLinux = 'Y' -o $deleteLinux = 'y' ] ; then
		...
		custom_param_ram="-m "$(expr $availableRAM - 2500 )"M"
		format=""
		mounted=1
	else
